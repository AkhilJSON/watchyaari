...
exports.forgotPassword = async function (req, res) {
    try {
        ...
        let user = await User.findOne({ email: body.email, isDeleted: { $ne: true } }, '_id email fullName passwordResetCode');
        if (!user) {
            return res.json({
                Success: false,
        ...
        User.findOne({ _id: mongoose.Types.ObjectId(decoded._id) }, async function (err, user) {
            ...
            if (user) {
                if (!user.emailVerified) {
                    await User.findByIdAndUpdate({ _id: user._id }, { $set: { emailVerified: true, emailVerifiedOn: new Date().getTime(), emailVerificationCode: '' } })
