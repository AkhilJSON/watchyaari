...
export class JoinPartyComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    if (partyId) {
      this.joinForm.controls.partyURL.setValue(
  ...
  joinParty() {
    if (this.joinForm.invalid) {
      this.openSnackBar(
        `Please provide valid Party URL to join`,
        '',
        'bottom',
        'center'
      );
      ...
      .subscribe((response: any) => {
        if (response && response.Success) {
          ...
          if (partyData.isEnded) {
            this.openSnackBar(
              `Party you are trying to enter has been ended at ${moment(
                partyData.endedOn
  ...
  getRecentPartyList() {
    ...
    let getPartyListResource = this.ps
      .getRecentPartyList({})
      .subscribe((response: any) => {
  ...
  onCopyingInviteURL() {
    this.openSnackBar(
      `Invite URL copied successfully!`,
      '',
      'bottom',
      'center'
    );
