...
export class LaunchPartyComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    let paramsService = this.activatedRoute.params.subscribe((params) => {
      ...
      let partyDataService = this.partyService
  ...
  setScheuledDate(event: any) {
    this.partyForm.controls.scheduledDate.setValue(
      moment(event.value).format('MMMM Do YY, h:mm a')
    );
  }
  ...
  launchParty() {
    ...
    if (formControls.status.value == 'SCHEDULED') {
      ...
      let scheduledDate = <any>(
        moment(formControls.scheduledDate.value, 'MMMM Do YY, h:mm a').format()
      );
    ...
    this.displayProgressSpinner = true;
    let launchPartyService = this.partyService
      .launchParty(body)
      .subscribe((res: any) => {
