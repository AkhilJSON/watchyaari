...
export class InviteGuestsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let sharedService = this.ss.partyDetails.subscribe((data: any) => {
      ...
      _.each(this.partyData.guests, (guest: any) => {
        if (guest && guest.userId) {
          guest.userId.isHost =
            guest.userId._id == this.partyData.hostedBy ? true : false;
          guest.userId.self =
            guest.userId._id == this.loggedInUserdId ? true : false;
    ...
    this.modalData.socket.on('UNBLOCK_USER', (userId: any) => {
      ...
      this.openSnackBar(
        `Host has updated the guest list.`,
        '',
        'bottom',
        'center'
      );
  ...
  remove(userId: any): void {
    if (!this.modalData.isHost) {
      this.openSnackBar(
        `Only host can block/remove the user.`,
        '',
        'bottom',
        'center'
      );
  ...
  selected(event: MatAutocompleteSelectedEvent) {
    ...
    if (isAlreadyExists) {
      ...
      this.openSnackBar(
        `User is blocked, unblock user & add.`,
        'close',
        'bottom',
        'center',
        4000
      ...
      guests.length ==
      (this.partyData.maxGuestsAllowed ||
        GlobalConstants.DEFAULT_MAX_GUESTS_ALLOWED)
    ) {
      this.openSnackBar(
        `The Party Room is full.`,
        'close',
        'bottom',
        'center',
        4000
  ...
  unblockUser(userId: any) {
    if (!this.modalData.isHost) {
      this.openSnackBar(
        `Only host can unblock the user.`,
        '',
        'bottom',
        'center'
      );
    ...
    this.partyService
      .unBlockUsers({ partyId: this.modalData.partyId, userId })
      .subscribe((response: any) => {
        this.updatingParty = false;
  ...
  fetchBlockedUsers() {
    ...
    this.partyService
      .fetchBlockedUsers({ partyId: this.modalData.partyId })
      .subscribe((response: any) => {
  ...
  updateCoHosts(guestId: any, guestName: any, add: any) {
    ...
    if (!this.modalData.isHost) {
      this.openSnackBar(
        `Only host can update CO-HOST role.`,
        '',
        'bottom',
        'center'
      );
    ...
    */
    let alreadyHaveOneCoHost = <any>_.filter(
      this.partyData.guests,
      (guest: any) => {
        return guest.isCoHost;
      }
    );
    alreadyHaveOneCoHost =
      alreadyHaveOneCoHost && alreadyHaveOneCoHost.length == 1 ? true : false;

    if (add && alreadyHaveOneCoHost) {
      this.openSnackBar(
        `A party can have only 1 CO-HOST.`,
        '',
        'bottom',
        'center'
      );
