...
export class ChangeVideoTrackComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        let profileDataService = this.ss.loggedInUserProfile.subscribe((profile: any) => {
            if (profile) {
    ...
    searchVideos(defaultSearch: any, search?: any) {
        ...
        let selectedProviders = _.filter(this.videoServiceProviders, (provider: any) => {
    ...
    launchByURL() {
        ...
        //Validate URL
        let isValidURL = this.ps.validateURL(this.searchURL.value, MediaPlayerConstants.TYPE.YOUTUBE);

        if (!this.searchURL.value || !isValidURL) {
            this.openSnackBar(`Please enter valid youtube video URL`, "", "bottom", "center");
            return;
        }
    ...
    useThisVideo(flow: any, videoData?: any) {
        ...
        let updateVideoService = this.ps.updateVideoInTheParty(newVideoData).subscribe((res: any) => {
            ...
            if (res.data) {
                ...
                let loggedInUserName = (this.loggedInUserData && this.loggedInUserData.fullName) || "Someone";
                ...
                this.openSnackBar(`Video updated successfully.`, "", "bottom", "center");
            }
        });
