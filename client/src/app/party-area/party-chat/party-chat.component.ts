...
export class PartyChatComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.chatSocket = io.connect(
      environment.defaultSocketConnection + '/partyChat',
    ...
    let userDetailsService = this.ps
      .getUserDetails({})
      .subscribe((response: any) => {
        if (response.Success && response.data) {
  ...
  sendChat() {
    ...
    enterCheckIndex >= 0 &&
      (this.chatText = this.chatText.substr(0, enterCheckIndex));
  ...
  toggleMessageScroll(status: any) {
    ...
    if (!status) {
      editor &&
        editor[0] &&
        editor[0].classList.remove('ql-editor-chat-area-no-scroll');
      return;
    }
    editor &&
      editor[0] &&
      editor[0].classList.add('ql-editor-chat-area-no-scroll');
  }
