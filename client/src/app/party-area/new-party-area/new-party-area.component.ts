...
export class NewPartyAreaComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
    ...
    constructor(
        ...
        let paramsService = this.activatedRoute.params.subscribe((params) => {
            ...
            //Socket connection
            this.socket = io.connect(environment.defaultSocketConnection + "/partySync", {
                query: `partyId=${this.partyId}`,
            });
            this.videoAudioSocket = io.connect(environment.defaultSocketConnection + "/videoAudioStream", {
                query: `partyId=${this.partyId}`,
            });
        });
        ...
        let youtubePlayerService = this.ss.youtubePlayer.subscribe((data: any) => {
            this.player = data;
            this.totalVideoDuration = this.playerControls.getDuration(MediaPlayerConstants.TYPE.YOUTUBE, {
                player: this.player,
            });
            setInterval(() => {
                if (this.triggerSeeking) {
                    this.currentSeek = this.playerControls.getCurrentTime(MediaPlayerConstants.TYPE.YOUTUBE, {
                        player: this.player,
                    });
                }
            }, 0);
    ...
    ngOnInit(): void {
        ...
        this.breakpointObserver
            ...
            .subscribe((result) => {
                ...
                this.configureBroadcasting(this.localAudioEnabled, this.localVideoEnabled);
        ...
        this.socket.on("CURRENT_PLAYER_STATUS", (currentSeek: any, partyStatus: any) => {
            // console.log({ currentSeek, partyStatus })
            this.currentSeek = currentSeek;
            this.partyStatus = partyStatus;

            let playerInterval = setInterval(() => {
                if (this.player && this.player.g && this.player.g.g && this.player.g.g.host) {
        ...
        this.socket.on("PUSH_NOTIFICATION", (message: any, specialMessageData: any) => {
            if (message) {
                this.openSnackBar(`${message}`, "", "bottom", "center");
            } else {
                if (specialMessageData.type == "roleUpdate") {
                    let isLoggedInUser = specialMessageData.guestUserId == this.loggedInUserdId;
                    if (specialMessageData.isAdd) {
                        message = ` ${
                            isLoggedInUser ? "YOU have " : specialMessageData.guestName + " has "
        ...
        this.socket.on("MY_CURRENT_SEEK", (seek: any, userId: any) => {
            ...
            this.connectedUsersSeekMap[this.loggedInUserdId] = parseInt(this.currentSeek);
            ...
            this.enableCatchup = highestSeekValue - this.currentSeek > 3 ? true : false;
            ...
            if (this.localVideoStatus == "loaded") {
                try {
                    ...
                    if (environment.production && this.iceServerCredentials) {
                        peerConnection = <RTCPeerConnection>new RTCPeerConnection(this.iceServerCredentials);
                    }
                    ...
                    peerConnection
                        ...
                        .then(() => {
                            const desc = {
                                ...
                                sdp: this.updateBandwidthRestriction(peerConnection.localDescription.sdp, 250),
                            };
        ...
        this.videoAudioSocket.on("broadcastData", (userId: any, broadcastData: any) => {
            // console.log(broadcastData)
            _.each(this.allParticipants, (participant: any) => {
                if (participant.userId == userId) {
                    participant.loading =
                        broadcastData && broadcastData.status == GlobalConstants.STATUS_MAP.LOADING ? true : false;
                    participant.hasVideo = broadcastData && broadcastData.video ? true : false;
                    participant.hasAudio = broadcastData && broadcastData.audio ? true : false;
    ...
    openChangeVideoModal() {
        if (!this.hasAccessForVideoControl) {
            this.openSnackBar(`Only Host or co-host can change the video.`, "", "bottom", "center");
            return;
        }
    ...
    partyAreaStatus() {
        ...
        if (this.userSessionAuthenticated == "unauthorised") {
            return {
                message: "You are already in an active party. Click continue to proceed in the current browser tab.",
    ...
    hostJoinedTheParty(response: any) {
        ...
        if (token && token.length && !isExpired) {
            this.cs.setItem("crntPrtyId", response.partyId, expirationDate, "/", null, null);
            this.cs.setItem(
    ...
    */
    ...
    partStartsIn() {
        ...
        let minutes = (diff("minutes", scheduledDate) % 60).toString().padStart(2, "0");
        let seconds = (diff("seconds", scheduledDate) % 60).toString().padStart(2, "0");
    ...
    seekVideo(event: any) {
        ...
        if (!this.hasAccessForVideoControl) {
            if (
                [MediaPlayerConstants.PLAYER_STATUS.paused, MediaPlayerConstants.PLAYER_STATUS.unstarted].includes(
                    this.playerStatus
                )
            ) {
                this.currentSeek = 0;
        ...
        if (this.hasAccessForVideoControl) {
            if (event.data == YT.PlayerState.PAUSED) {
                ...
                if (!this.isExternalRequestToPlayPause) {
                    ...
                    this.socket.emit("PAUSE_PARTY", JSON.stringify({ hostSessionId, seekTo: this.currentSeek }));
                }
                this.isExternalRequestToPlayPause = false;
            }
            ...
            if (event.data == YT.PlayerState.PLAYING) {
                ...
                if (!this.isExternalRequestToPlayPause) {
                    this.updatePartyPlayerStatus(MediaPlayerConstants.PARTY_STATUS.RESUME);
                    this.socket.emit("RESUME_PARTY", JSON.stringify({ hostSessionId, seekTo: this.currentSeek }));
                }
                this.isExternalRequestToPlayPause = false;
            }
        } else {
            if ([YT.PlayerState.PAUSED, YT.PlayerState.PLAYING].includes(this.playerStatus)) {
                ...
                if (YT.PlayerState.PAUSED == this.playerStatus) {
                    if (!this.hasPermissionToPause && this.partyStatus == MediaPlayerConstants.PARTY_STATUS.PAUSE) {
                        this.hasPermissionToResume = true;
                        this.playerControls.playVideo(MediaPlayerConstants.TYPE.YOUTUBE, {
                            player: this.player,
                        });
                ...
                if (YT.PlayerState.PLAYING == this.playerStatus) {
                    if (!this.hasPermissionToResume && this.partyStatus == MediaPlayerConstants.PARTY_STATUS.RESUME) {
                        this.playerControls.pauseVideo(MediaPlayerConstants.TYPE.YOUTUBE, {
                            player: this.player,
                            status: this.playerStatus,
                        });
    ...
    changeVolume(event: any) {
        ...
        this.volumeButton.icon = volume ? (volume > 20 ? "volume_up" : "volume_down") : "volume_off";
    ...
    updateParticipantsData(userId: any, status?: any) {
        _.each(this.allParticipants, (participant: any) => {
            if (participant.userId == userId) {
                status && _.has(status, "video") && (participant.hasVideo = status.video);
                status && _.has(status, "audio") && (participant.hasAudio = status.audio);
                status && _.has(status, "loading") && (participant.loading = status.loading);
                status && _.has(status, "online") && (participant.online = status.online);
            }
        });
    }
        ...
        setTimeout(() => {
            ...
            navigator.mediaDevices
                ...
                .catch((error) => {
                    ...
                    this.openSnackBar(`Error occurred accessing hardware. Please try again.`, "", "", "center");
                    this.videoAudioSocket.emit("broadcastData", {
                        audio: false,
                        video: false,
                    });
    ...
    toggleChatInSmallScreens() {
        this.chatPreference == "video" ? (this.chatPreference = "text") : (this.chatPreference = "video");
    }
    ...
    closeAllPeerConnections() {
        ...
        _.each(allConnections, (connection) => {
            this.peerConnections[connection] && this.peerConnections[connection].close();
        });
    }
    ...
    closeOtherPartySessions(event: any) {
        ...
        if (event.closeOtherSessions) {
            this.socket.emit("RELEASE_ACTIVE_PARTY_SESSION_LOCK", this.loggedInUserdId);
        }
    }
